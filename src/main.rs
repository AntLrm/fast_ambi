...
impl ScreenSide {
    ...
    fn get_linear_from_border_coord(&self, screen: &Screen, border_coord: u32) -> u32 {
        match self {
            ...
            ScreenSide::Bottom | ScreenSide::Left => {
                self.get_offset(screen) + self.get_side_res(screen) - border_coord - 1
            }
        }
    }
}
    ...
    fn convert_to_linear_coord(&self, side: &ScreenSide, screen: &Screen) -> u32 {
        match side {
            ScreenSide::Top | ScreenSide::Bottom => {
                side.get_linear_from_border_coord(screen, self.x)
            }
            ScreenSide::Right | ScreenSide::Left => {
                side.get_linear_from_border_coord(screen, self.y)
            }
        }
    }
}
...
impl Box {
    ...
    fn get_linear_coord(&self, screen: &Screen) -> (u32, u32) {
        let box_linear_start = self
            .screen_start
            .convert_to_linear_coord(&self.side, screen);
        let box_linear_end = self.screen_end.convert_to_linear_coord(&self.side, screen);
        (
            cmp::min(box_linear_start, box_linear_end),
            cmp::max(box_linear_start, box_linear_end),
        )
    }
}
...
fn get_leds(boxes: &[Box], x_led_count: u32, y_led_count: u32, screen: &Screen) -> Vec<Led> {
    ...
    for idx in 0..x_led_count {
        ...
        let led_linear_pos =
            ScreenSide::Top.get_linear_from_border_coord(screen, led_pos_on_border);
        leds.push(Led::new(&boxes, led_linear_pos, screen));
    }
    ...
    for idx in 0..y_led_count {
        ...
        let led_linear_pos =
            ScreenSide::Right.get_linear_from_border_coord(screen, led_pos_on_border);
        leds.push(Led::new(&boxes, led_linear_pos, screen));
    }
    ...
    for idx in (0..x_led_count).rev() {
        let led_pos_on_border: u32 = (screen.x_res / x_led_count) * idx;
        let led_linear_pos =
            ScreenSide::Bottom.get_linear_from_border_coord(screen, led_pos_on_border);
        leds.push(Led::new(&boxes, led_linear_pos, screen));
    }
    ...
    for idx in (0..y_led_count).rev() {
        let led_pos_on_border: u32 = (screen.y_res / y_led_count) * idx;
        let led_linear_pos =
            ScreenSide::Left.get_linear_from_border_coord(screen, led_pos_on_border);
        leds.push(Led::new(&boxes, led_linear_pos, screen));
    }
...
fn get_distance_to_border(point: &ScreenCoord, side: &ScreenSide, screen: &Screen) -> u32 {
    match side {
        ScreenSide::Top     => point.y,
        ScreenSide::Bottom  => screen.y_res - 1 - point.y,
        ScreenSide::Left    => point.x,
        ScreenSide::Right   => screen.x_res - 1 - point.x,
    }
}


fn color_boxes(boxes: &mut Vec<Box>, screen: &Screen, sample_count: u32, sample_depth: u32) {
    boxes.iter_mut().for_each(|b| {
        let mean_color: Vec<u32> = (0..sample_count)
            .collect::<Vec<u32>>()
            .iter()
            .map(|&idx| {
                let sampling_pixel = ScreenCoord {
                        x: b.sample_points[idx as usize].x,
                        y: b.sample_points[idx as usize].y,
                    };

                let distance_to_border = get_distance_to_border(&sampling_pixel, &(b.side), screen);
                let relative_distance_to_border = (100 * (sample_depth - distance_to_border) / sample_depth) as u16;

                let screen_capture =
                    screen.capture_ximage(1, 1, sampling_pixel.x as i32, sampling_pixel.y as i32);
                ...
                return [(relative_distance_to_border * (clr.red / 256)) /100, (relative_distance_to_border * (clr.green / 256)) / 100, (relative_distance_to_border * (clr.blue / 256)) / 100];
            })
...
impl Led {
    fn new(boxes: &[Box], linear_position: u32, screen: &Screen) -> Self {
        let (idx, _) = boxes
            .iter()
            .enumerate()
            .find(|(_, b)| {
                b.get_linear_coord(screen).0 <= linear_position
                    && b.get_linear_coord(screen).1 >= linear_position
            })
            .unwrap();
    ...
    fn update_color(
        ...
        screen: &Screen,
        luminosity: u32,
        color_correction_red: u32,
        color_correction_green: u32,
        color_correction_blue: u32,
    ) {
        ...
        let current_box_center =
            current_box_linear.0 + (current_box_linear.1 - current_box_linear.0) / 2;

        let (b1, b2, relative_unit) = match self.linear_position.cmp(&current_box_center) {
            cmp::Ordering::Less => (
                previous_box,
                current_box,
                100 * (self.linear_position - current_box_linear.0)
                    / (current_box_center - current_box_linear.0),
            ),
            cmp::Ordering::Greater | cmp::Ordering::Equal => (
                current_box,
                next_box,
                100 * (self.linear_position - current_box_center)
                    / (current_box_linear.1 - current_box_center),
            ),
        };

        self.color = CRGB {
            r: (luminosity
                * (color_correction_red
                    * ((relative_unit * b2.color.r as u32
                        + (100 - relative_unit) * b1.color.r as u32)
                        / 100)
                    / 100)
                / 100) as u8,
            g: (luminosity
                * (color_correction_green
                    * ((relative_unit * b2.color.g as u32
                        + (100 - relative_unit) * b1.color.g as u32)
                        / 100)
                    / 100)
                / 100) as u8,
            b: (luminosity
                * (color_correction_blue
                    * ((relative_unit * b2.color.b as u32
                        + (100 - relative_unit) * b1.color.b as u32)
                        / 100)
                    / 100)
                / 100) as u8,
        };
...
fn write_to_serial(
    ...
    for idx in 0..leds.len() {
        ...
        values.push(std::cmp::min(
            TryInto::<u8>::try_into(leds[(idx + starting_led as usize) % leds.len()].color.r)
                .unwrap(),
            252,
        )); //led color is
            //capped at 252
            ...
            //    headers.
        values.push(std::cmp::min(
            TryInto::<u8>::try_into(leds[(idx + starting_led as usize) % leds.len()].color.g)
                .unwrap(),
            252,
        )); //led color is
        values.push(std::cmp::min(
            TryInto::<u8>::try_into(leds[(idx + starting_led as usize) % leds.len()].color.b)
                .unwrap(),
            252,
        )); //led color is
    }
...
fn main() {
    ...
    let luminosity_percent = 35;
    let border_depth = 400;
    let color_correction_red = 100;
    ...
    let x_box_cnt = 9;
    let y_box_cnt = 5;
    let sampling_size = 13;
    let loop_min_time = 20;
    ...
    loop {
        ...
        color_boxes(&mut boxes, &screen, sampling_size, border_depth);
        color_leds(
            &mut leds,
            &boxes,
            &screen,
